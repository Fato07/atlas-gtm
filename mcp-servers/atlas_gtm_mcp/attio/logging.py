"""Structured JSON logging for Attio MCP tools.

Provides structured logging with required fields per FR-005:
- tool: Tool name
...
def log_tool_error(
    ...
    log.error(
        ...
        error_message=str(error)[:200],  # Truncate error message
        correlation_id=correlation_id,
    )
...
def log_api_call(
    ...
    if error:
        ...
        log.warning("attio_api_call", **log_data)
    else:
        log.debug("attio_api_call", **log_data)
